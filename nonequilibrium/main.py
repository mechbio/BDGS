...
# -----------------------------------------------------------------------------
# This file (main.py) generates equilibrium drop from the nonequilibrium drop
# -----------------------------------------------------------------------------

# date         :21-Jun-22
# version      :0.1.0
# usage        :python3 main.py 0.2 1.2 0.8 0.45 0.1 0.5 0.1
# py_version   :3.8
...
# Initialization
h1init = float(sys.argv[5])
h2init = float(sys.argv[6])
h3init = float(sys.argv[7])

# Nonequilibrium solution
sol = sym.nsolve([eqn2,eqn3,eqn4],(h1,h2,h3),(h1init,h2init,h3init))
# print(sol)
h1sol,h2sol,h3sol=sol
R1sol=(h1sol**2+Rb**2)/(2*h1sol)
R2sol=(h2sol**2+Rb**2)/(2*h2sol)
R3sol=(h3sol**2+Rb**2)/(2*h3sol)
...
with open('data/drop.pkl','wb') as f:
    pickle.dump([alpha,beta2,beta3,Rb,h1sol,h2sol,h3sol], f)
